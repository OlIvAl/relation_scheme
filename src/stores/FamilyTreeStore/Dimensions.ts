export default
class Dimensions {
  static VIEWPORT_WIDTH: number = 900;
  static VIEWPORT_HEIGHT: number = 750;

  static TARGET_ELEMENT_STROKE_WIDTH: number = 5;
  static TARGET_ELEMENT_HEIGHT: number = 80;
  static ELEMENT_HEIGHT: number = 70;
  static ELEMENT_BLOCK_PADDING: number = 10;
  static ELEMENT_FONT_SIZE: number = 16;
  static ROLE_HEIGHT: number = 30;

  static INDENTS_BETWEEN_ELEMENTS: number = 50;
  static INDENTS_BETWEEN_LEVELS: number = 40;
  static INDENTS_BETWEEN_ROLES: number = 0;

  static ELEMENTS_IN_LEVEL: number = 4;

  static ITEM_BUBBLE_X_INDENT: number = 25;
  static ITEM_BUBBLE_Y_INDENT: number = 15;

  static ARROW_BUBBLE_X_INDENT: number = 7;
  static ARROW_BUBBLE_Y_INDENT: number = 5;

  static BUBBLE_RIGHT_LEFT_PADDING: number = 35;
  static BUBBLE_TOP_BOTTOM_PADDING: number = 30;

  static ARROW_STROKE_WIDTH: number = 10;
  static ARROW_STRONG_STROKE_WIDTH: number = Dimensions.ARROW_STROKE_WIDTH * 2;
  static ARROW_TRACK_STROKE_WIDTH: number = Dimensions.ARROW_STROKE_WIDTH * 3;

  static ACTION_BUTTON_SIZE: number = 25;

  static TIMER: number = 300;
}